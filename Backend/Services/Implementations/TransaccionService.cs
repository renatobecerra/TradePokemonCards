...
namespace Backend.Services.Implementations
{
    public class TransaccionService : ITransaccionService
    {
        ...
        public TransaccionService(PokemonMarketContext context)
        {
            _context = context;
        }

        public async Task<(bool exito, string mensaje)> ProponerTratoAsync(ProponerTratoDto dto)
        {
            ...
            if (invUser == null)
                return (false, "Carta no encontrada en el inventario del vendedor.");
            ...
            return (true, "Propuesta enviada al chat.");
        }

        public async Task<(bool exito, string mensaje)> ConfirmarTratoAsync(ProponerTratoDto dto)
        {
            var invUser = await _context.InventarioUsuarios
                .FirstOrDefaultAsync(i => i.IdInventarioUser == dto.IdInventarioUser && i.IdUsuario == dto.IdVendedor);

            if (invUser == null || invUser.Cantidad <= 0)
                return (false, "La carta ya no está disponible.");
            ...
            return (true, "Trato confirmado exitosamente.");
        }

        public async Task<bool> VerificarTransaccionCompletadaAsync(int idVendedor, int idComprador)
        {
